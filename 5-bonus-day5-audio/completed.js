...
// Complete game with visual effects and immersive audio

// All Day 4 variables plus audio enhancements
let player;
let gameState = "start";
let bullets = [];
let enemies = [];
...
function setup() {
  ...
  // Initialize audio oscillators for procedural sound
  initializeAudio();
...
function draw() {
  ...
  // Audio-reactive animated background
  drawAudioReactiveBackground();
  
  if (gameState === "start") {
    // Enhanced start screen with audio visualization
    ...
    text("Press M to toggle sound", width/2, height/2 + 150);
    
    // Draw audio visualization on start screen
    drawAudioVisualization(width/2, height/2 + 200, 200, 30);
    ...
    // Weapon-based shooting with sound
    let fireRate = 15;
    ...
    // Update and draw bullets with audio-reactive trails
    for (let i = bullets.length - 1; i >= 0; i--) {
      ...
      // Audio-reactive bullet trails
      let audioIntensity = getAudioIntensity();
      for (let j = 0; j < bullet.trail.length; j++) {
        let alpha = (j / bullet.trail.length) * (100 + audioIntensity * 50);
        let size = 4 - j/2 + audioIntensity * 2;
      ...
      // Draw main bullet with audio pulse
      let bulletSize = 6 + audioIntensity * 3;
      if (bullet.type === "basic") {
        fill(255, 255, 0);
        ellipse(bullet.x, bullet.y, bulletSize + 2);
        fill(255, 255, 150);
        ...
        fill(0, 255, 255);
        ellipse(bullet.x, bullet.y, bulletSize - 1);
      }
    ...
    // Spawn enemies with audio-reactive spawn rate
    let baseSpawnRate = 1.2;
    let levelBonus = (playerLevel - 1) * 0.4;
    let timeBonus = difficultyTimer / 2400;
    let audioBonus = getAudioIntensity() * 0.5; // Audio makes more enemies spawn
    let enemySpawnChance = baseSpawnRate + levelBonus + timeBonus + audioBonus;
    ...
    // Update and draw enemies with audio-reactive animation
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      // Audio-reactive enemy pulsing
      enemy.pulsePhase += 0.1 + getAudioIntensity() * 0.2;
      let pulseSize = enemy.size + sin(enemy.pulsePhase) * (3 + getAudioIntensity() * 5);
    ...
    // Update and draw power-ups with audio-reactive glow
    for (let i = powerUps.length - 1; i >= 0; i--) {
      let powerUp = powerUps[i];
      powerUp.glowPhase += 0.05 + getAudioIntensity() * 0.1;
      
      let glowSize = powerUp.size + sin(powerUp.glowPhase) * (5 + getAudioIntensity() * 8);
    ...
    // Draw player with audio-reactive glow
    push();
    translate(player.x, player.y);
    rotate(player.rotation);
    
    let audioGlow = 8 + getAudioIntensity() * 12;
    fill(0, 255, 255, 100);
    ellipse(0, 0, player.size + audioGlow);
    ...
    // Draw enhanced UI with audio visualization
    drawEnhancedUI();
    drawAudioVisualization(10, height - 60, 200, 40);
...
function initializeAudio() {
  // Initialize procedural audio oscillators
  // In a real implementation with p5.sound, you would create actual oscillators here
  // For now, we simulate the audio system
}

function updateBackgroundMusic() {
  // Update background music intensity based on game state
  let targetVolume = musicVolume * (1 + enemies.length * 0.1);
  // Adjust music based on action intensity
}

function stopBackgroundMusic() {
  // Stop or fade out background music
}

function updateAudioVisualization() {
  // Simulate audio frequency data
  for (let i = 0; i < audioVisualization.length; i++) {
    audioVisualization[i] = random(0, 1) * (enemies.length * 0.2 + 0.3);
  }
}
...
function getAudioIntensity() {
  ...
  let sum = 0;
  for (let i = 0; i < audioVisualization.length; i++) {
    sum += audioVisualization[i];
  }
  return sum / audioVisualization.length;
}

function drawAudioReactiveBackground() {
  let baseHue = frameCount * 0.1;
  background(15 + getAudioIntensity() * 10, 15 + getAudioIntensity() * 5, 30);
  
  // Audio-reactive moving stars
  for (let i = 0; i < 50; i++) {
    let x = (i * 77) % width;
    let y = (i * 43 + frameCount * 0.5) % height;
    let brightness = 100 + sin(frameCount * 0.02 + i) * 50 + getAudioIntensity() * 100;
    let size = 2 + getAudioIntensity() * 3;
...
function drawEnhancedUI() {
  // Health bar with audio-reactive glow
  let audioGlow = getAudioIntensity() * 50;
  fill(255, 0, 0, 100 + audioGlow);
  rect(8, 8, 204, 24);
  fill(255, 0, 0);
  rect(10, 10, 200, 20);
  fill(0, 255, 0);
  ...
  // XP bar with audio-reactive glow
  fill(100, 100, 255, 100 + audioGlow);
  rect(8, 33, 204, 19);
  fill(100, 100, 255);
  rect(10, 35, 200, 15);
  fill(0, 255, 255);
  ...
  // Text with audio-reactive effects
  fill(255);
  textAlign(LEFT, TOP);
  textSize(16);
  text("Health", 220, 12);
  ...
  text("Sound: " + (soundEnabled ? "ON" : "OFF"), 10, 120);
  
  textAlign(RIGHT, TOP);
  text("1,2,3: Weapons | M: Sound", width - 10, 10);
}

// Audio functions (procedural sound simulation)
function playShootSound() {
  if (!soundEnabled) return;
  // Simulate shooting sound
}

function playExplosionSound() {
  if (!soundEnabled) return;
  // Simulate explosion sound
}

function playPowerUpSound() {
  if (!soundEnabled) return;
  // Simulate power-up collection sound
}

function playDamageSound() {
  if (!soundEnabled) return;
  // Simulate damage taken sound
}

function playLevelUpSound() {
  if (!soundEnabled) return;
  // Simulate level up sound
}
