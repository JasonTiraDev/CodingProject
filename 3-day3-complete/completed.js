...
// We start with working Day 2 code and add health + power-ups
...
let maxHealth = 3;             // Maximum health is 3
  ...
  if (gameState === "start") {
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      if (distance < (player.size + enemy.size) / 2) {
        playerHealth -= 1;          // Lose 1 health
        enemies.splice(i, 1);       // Remove the enemy

        if (playerHealth <= 0) {    // Check if dead
          gameState = "gameOver";
        }
      }
    ...
    if (random(1000) < 1) {         // 0.1% chance each frame - very rare!
      powerUps.push({
        x: random(30, width - 30),  // Random position on screen
    ...
    for (let i = powerUps.length - 1; i >= 0; i--) {
      ...
      // Check if player touches health pack
      let dist = sqrt((player.x - powerUp.x) ** 2 + (player.y - powerUp.y) ** 2);
      if (dist < (player.size + powerUp.size) / 2) {
        playerHealth = min(playerHealth + 1, maxHealth);  // Heal 1 health, max 3
        powerUps.splice(i, 1);                            // Remove health pack
      }
    }
    ...
    for (let i = 0; i < playerHealth; i++) {
      text("❤️", 10 + i * 30, 35);  // Draw hearts side by side
    }
...
// 11. Draw health as HEARTS instead of numbers (❤️❤️❤️) - DONE!
